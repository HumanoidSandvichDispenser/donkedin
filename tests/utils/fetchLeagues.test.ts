...
describe("fetchLeagues utils", () => {
  describe("fetchRglExternalData", () => {
    it("returns null when profile call fails", async () => {
      const client: any = {
        profile: {
          getV0Profile: vi.fn(() => Promise.reject(new Error("not found"))),
          getV0ProfileTeams: vi.fn(),
        },
      };

      const res = await fetchRglExternalData(client, "123");
      expect(res).toBeNull();
      expect(client.profile.getV0Profile).toHaveBeenCalledWith("123");
      expect(client.profile.getV0ProfileTeams).not.toHaveBeenCalled();
    });
    ...
    it("returns profile and mapped teams when both calls succeed", async () => {
      ...
      const res = await fetchRglExternalData(client, "999");
      expect(res).not.toBeNull();
      expect(res?.steamId).toBe("S64");
      expect(res?.name).toBe("RGLGuy");
  ...
  describe("fetchEtf2lExternalData", () => {
    it("returns null when profile call fails", async () => {
      const client: any = {
        player: {
      ...
      const res = await fetchEtf2lExternalData(client, "42");
      expect(res).toBeNull();
    ...
    it("concatenates teams from multiple pages of transfers", async () => {
      ...
      const res = await fetchEtf2lExternalData(client, "777");
      expect(res).not.toBeNull();
    ...
    it("handles some page fetches failing and still returns concatenated successful pages", async () => {
      ...
      const res = await fetchEtf2lExternalData(client, "88");
      expect(res).not.toBeNull();
      expect(res?.teams).toEqual([
